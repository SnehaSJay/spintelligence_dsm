-- Resets AFIS-6 Cotton Data Entry table to match the new payload shape:
-- adds lot_no, variety, invoice_date, mc_name, blow_room, carding,
-- breaker_drawing, finisher_drawing, comber, scp_nep_count; drops the old
-- material_class/comment/nep-detail/fineness/maturity/ifc columns.
CREATE SCHEMA IF NOT EXISTS mixing;

DROP TABLE IF EXISTS mixing.afis6_cotton_data_entry CASCADE;

CREATE TABLE mixing.afis6_cotton_data_entry (
...
CREATE UNIQUE INDEX afis6_cotton_data_entry_entry_id_uq
    ON mixing.afis6_cotton_data_entry (entry_id)
    WHERE entry_id IS NOT NULL;

CREATE INDEX afis6_cotton_data_entry_inspection_date_idx
